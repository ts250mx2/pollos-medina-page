/**
 * Categorías, productos y sus opciones.
 */
"use strict";
...
/**
 * Menú completo con la misma forma que consume el sitio público.
 * @param {boolean} soloActivos - true para el sitio, false para el panel.
 */
async function menuCompleto(soloActivos = true) {
  ...
  const elecciones = await consultar(
  ...
  return categorias.map((c) => ({
    ...
    productos: (productosPorCategoria[c.id] || []).map((p) => ({
      id: p.slug,
      idNumerico: p.id,
      nombre: p.nombre,
      desc: p.descripcion || "",
      ...
      opciones: (opcionesPorProducto[p.id] || []).map((o) => ({
        id: o.slug,
        label: o.etiqueta,
        elecciones: (eleccionesPorOpcion[o.id] || []).map((e) => ({
          id: e.slug,
          label: e.etiqueta,
...
function agrupar(filas, campo) {
  return filas.reduce((acc, fila) => {
    (acc[fila[campo]] = acc[fila[campo]] || []).push(fila);
    return acc;
  }, {});
}
...
function leerDatosProducto(datos) {
...
async function actualizarProducto(id, datos) {
  ...
  if (repetido) throw new ErrorHttp(409, `Ya existe otro producto con el identificador "${p.slug}".`);
...
/** Borra las opciones del producto y las vuelve a insertar según lo enviado. */
async function reemplazarOpciones(cx, productoId, opciones) {
